...
//function calculateTotalPoints($final){
    $points = 0;
    $pointsYellow = 0;
    $pointsBrown = 0;

    if($_SESSION["gameModeWordle"] == 1 && $_SESSION["loseGameChronoByTime"] == true){
        echo "tiempo";
        $_SESSION[$_SESSION['user']."totalPointsUser"] = $points;
    }

    else if($_SESSION["gameModeWordle"] == 1){
        echo "intentos";
        foreach($_SESSION[$_SESSION['user']] as $array){
            $pointsYellow = $array[0] * -1;
            $pointsBrown = $array[1] * -2;
        
            if ($pointsYellow + $pointsBrown >= -150){
        ...
        if($final== "win"){
            echo "Win";
            //$points += $_SESSION['secPoints'];//puntos que obtienes con el cronometro en el modo normal
